(function() {
  ...
  Macro.add('setcoords', {
    handler: function() {
      ...
      // CORRECTION CRITIQUE : Gestion robuste du passage actuel
      const currentPassage = State.passage?.title || State.variables.currentPassage ||"PassageInconnu";
      console.log(`🎯 SETCOORDS: Passage "${currentPassage}" -> (${x}, ${y}, ${continent})`);
      ...
      v.playerCoordinates.passage = currentPassage;
  ...
  Macro.add('pnjfollow', {
    handler: function() {
      ...
      // CORRECTION CRITIQUE : Passage actuel sécurisé
      const currentPassage = State.passage?.title || State.variables.currentPassage ||'PassageInconnu';
      console.log(`📍 Passage actuel: "${currentPassage}"`);

      const v = V();

      // VALIDER les coordonnées actuelles du PNJ
  ...
  // Chargement asynchrone de la géographie
  async function loadGeography() {
    ...
    if (!success) {
      ...
      // Utiliser le cache de fallback
      window.setup.geographyState.data = window.setup.fallbackGeography;
      window.setup.geographyState.ready = true;
  ...
  // Fonction pour obtenir les données géographiques de façon sécurisée
  window.setup.getGeographyData = function() {
    // Si le système de géographie n'est pas prêt, utiliser le cache de fallback
    if (!window.setup.geographyState.ready) {
      console.warn("⚠️ Géographie pas prêt, utilisation du fallback");
      return window.setup.fallbackGeography;
    }

    return window.setup.geographyState.data || window.setup.fallbackGeography;
  };
  ...
  window.setup.startPNJTravel = function(pnjId, destinationPassage, destinationCoords, destinationContinent, travelType) {
    ...
    // VALIDATION PRÉALABLE : Assurer les coordonnées du PNJ
    ...
    if (!finalDestinationCoords || typeof finalDestinationCoords.x !== 'number') {
      finalDestinationCoords = window.setup.ensurePassageCoords(destinationPassage);
    }
    ...
    // CORRECTION : S'assurer que le PNJ a des coordonnées valides
    const currentCoords = npc.coordinates || {
      x: 0,
      y: 0
    };
    const currentContinent = npc.continent || "Eldaron";

    // Calcul de la distance avec les coordonnées VALIDÉES
    ...
    // Notification de départ avec réactions JSON
  ...
  window.setup.getLocationString = function(coords, continent) {
    ...
    // Récupérer les données géographiques de façon sécurisée
  ...
  window.setup.validatePNJCoordinates = function(pnjId) {
    ...
    // AJOUT: Validation du continent
    if (!npc.continent || typeof npc.continent !== 'string') {
      // Essayer de déterminer le continent par les coordonnées
      const geoData = window.setup.getGeographyData();
    ...
    if (!npc.passage && npc.isSpawned) {
      npc.passage = State.passage?.title || 'PassageInconnu';
    }

    console.log(`📍 Coordonnées validées pour ${pnjId}: (${npc.coordinates.x}, ${npc.coordinates.y}, ${npc.continent})`);
  ...
  window.setup.updateFollowersCoordinates = function() {
    ...
    // CORRECTION : Gestion robuste du passage actuel
    const currentPassage = State.passage?.title || State.variables.currentPassage ||'Geole';

    if (!currentPassage) {
      console.warn("⚠️ Aucun passage actuel défini dans updateFollowersCoordinates");
      return;
    }
    ...
    console.log(`🎯 Coordonnées joueur mises à jour: (${passageCoords.x}, ${passageCoords.y}, ${passageCoords.continent})`);
  };
  ...
  // FONCTION POUR CHARGER L'INDEX DES PNJs
  ...
  // FONCTION DE FALLBACK - Détection manuelle
  async function detectAvailablePNJs() {
    ...
    for (const pnjFile of knownPNJs) {
      try {
        ...
        // Fichier n'existe pas, ignorer silencieusement
      }
    }
  ...
  // FONCTION POUR OBTENIR LES DONNÉES PNJ DE MANIERE SÉCURISÉE
  window.setup.getPnjData = function(pnjId) {
    const pnjData = window.setup.loadPNJ(pnjId);

    // Extraction SÉCURISÉE selon votre structure
    return {
  ...
  // ------------------------------------------------------
  // INITIALISATION SÉCURISÉE - À PLACER EN TÊTE
  // ------------------------------------------------------
  // REMPLACER complètement le gestionnaire :storyready
  $(document).one(':storyready', function() {
    console.log("🎮 STORY READY - INITIALISATION SÉCURISÉE");

    // CORRECTION CRITIQUE : S'assurer que le passage actuel est défini
    State.variables.currentPassage = State.passage?.title || 'Geole';
    console.log(`🔧 State.variables.currentPassage initialisé à: ${State.variables.currentPassage}`);

    // Initialiser les variables de base
    window.setup.ensureBaseStats();

    // S'assurer que le passage actuel a des coordonnées IMMÉDIATEMENT
    const currentPassage = State.passage?.title || State.variables.currentPassage ||'Geole';
    window.setup.ensurePassageCoords(currentPassage);

    // Démarrer le chargement dans l'ordre
    Promise.resolve()
      .then(() => {
        console.log("🗺️ Démarrage chargement géographie...");
        return loadGeography();
      })
      .then(() => {
        console.log("📦 Démarrage chargement loot...");
        return loadLootsSequentially();
      })
      .then(() => {
        console.log("👥 Démarrage chargement PNJ...");
        return loadAllPNJ();
      })
      .catch(error => {
        console.error("❌ ERREUR D'INITIALISATION:", error);
        // Fallbacks si les chargements échouent
        initLootSystem();
        window.setup.pnjState.ready = true;
        window.setup.geographyState.ready = true;
      });
    ...
    window.setup.ensurePassageCoords = function(passageName) {
        ...
        // Utilise le passage actuel si passageName est undefined
        const actualPassageName = passageName || State.passage?.title || 'Geole';

        if (!v.passageCoords[actualPassageName]) {
            // Crée des coordonnées par défaut basées sur le joueur
  ...
  $(document).on(':passagedisplay', function() {
    State.variables.currentPassage = State.passage?.title;
    $('#passages').stop(true, true).animate({
      opacity: 1
    }, 400);
    window.setup.updateHUD();

    // CORRECTION : Valider les coordonnées du passage actuel
    const currentPassage = State.passage?.title;
    if (currentPassage) {
      window.setup.ensurePassageCoords(currentPassage);
    }
